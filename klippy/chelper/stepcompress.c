...
stepcompress_find_past_position(struct stepcompress *sc, uint64_t clock)
{
    ...
    list_for_each_entry(hm, &sc->history_list, node) {
        ...
        uint32_t clock_diff = clock - hm->first_clock, offset;
        clock_diff += hm->sm.interval;
        if (!hm->sm.add) {
            offset = clock_diff / hm->sm.interval;
        } else {
            // Solve for "count" using quadratic formula
            double add=hm->sm.add, interval=hm->sm.interval, ticks=clock_diff;
            double a = add/2., b = interval - add/2., c = -ticks;
